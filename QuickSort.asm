...
ints_to_str: # a0: address of int arr, a1: bytes size
	addi		$s7, $s7, 0		# s7 is the bytes count, will be return at $v1
	addi		$t7, $0, 0
	add		$t0, $0, $a0
	add		$v0, $0, $a2
	addi		$s0, $0, 0
...
huge_loop:
	...
	lw		$s1, 0($t1)
	slt		$t7, $s1, $0
	beq		$t7, $0, no_negative_num_process
	abs		$s1, $s1
no_negative_num_process:
	addi		$s0, $s0, 4
tiny_loop:
	addi		$s7, $s7, 1
	div		$s1, $t9
	...
	j		tiny_loop
string_process:
	beq		$t7, $0, not_negative_string_process
	addi		$t7, $0, 0
	addi		$s6, $0, 45
	add		$a2, $v0, $t8
	addi		$t8, $t8, 1
